...
public class Vehiculos {
	...
	public String verificaInicio(UsuarioDto usuario) {
		...
		query.append("CONCAT ('Tienes ', CONVERT((COUNT(sv.ID_VEHICULO)-COUNT(smvi.ID_MTTOVEHICULAR)),CHAR), ' Vehículos sin registrar la verificación al inicio de la jornada, te recordamos que debes registrar diariamente') as total_sin_verficacion \n");
	...
	public String programacionMantenimiento(UsuarioDto usuario) {
		...
		if (usuario.getIdOficina().equals(AppConstantes.NIVEL_CENTRAL)) {
	       ...
	       query.append("' Vehículos en estado ''Vence Hoy'' en la Programación de Mantenimiento, te recordamos que debes realizarlo') AS total FROM DUAL \n");
	       query.append("UNION ALL \n");
